#!/usr/bin/env python
"""Load raw data into a nicer format backed by HDF5.

2020.08.25 : Deprecated in favour of SQL database.
...
PROCESSED_DATA_PATH = Path().joinpath(
    'data', 'processed', 'trials_by_celltype'
)
...
for sess in rio.walk_sessions(RAW_DATA_PATH):
    ...
    try:
        with h5py.File(
            PROCESSED_DATA_PATH.joinpath(str(sess.day) + '.h5'), 'a'
        ) as f:
        ...
        print(
            'Failed while saving {} {} {}'.format(
                sess.cell_type, sess.day, sess.mouse_id
            )
        )
        raise
